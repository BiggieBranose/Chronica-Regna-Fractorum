...
void Game::init() {
    crf::Log::info("Game: initializing sprite system");
    createQuadBuffer();
    createTexture();
    ...
    createPipeline();
    crf::Log::info("Game: sprite system ready");
}
...
void Game::update(float dt) {
    ...
    if (m_window.isKeyPressed(GLFW_KEY_W)) dz -= speed * dt;
    if (m_window.isKeyPressed(GLFW_KEY_S)) dz += speed * dt;
    if (m_window.isKeyPressed(GLFW_KEY_A)) dx -= speed * dt;
    if (m_window.isKeyPressed(GLFW_KEY_D)) dx += speed * dt;

    if (dx != 0.0f || dz != 0.0f) {
        m_charX += dx;
        m_charZ += dz;
        m_charAngle = atan2f(dx, -dz);
    }

    m_charY = terrainHeight(m_charX, m_charZ);
}

void Game::render(VkCommandBuffer cmd, crf::u32 imageIndex, const float* viewPtr, const float* projPtr) {
    float ty = terrainHeight(m_charX, m_charZ);
    glm::mat4 model = glm::translate(glm::mat4(1.0f), glm::vec3(m_charX, ty + 0.5f, m_charZ));
...
void Game::createPipeline() {
    ...
    // Custom vertex input for SpriteVertex {vec2 pos, vec2 uv}
