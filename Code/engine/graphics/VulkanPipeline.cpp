...
void VulkanPipeline::createRayQueryDescriptorSetLayout() {
    VkDescriptorSetLayoutBinding uboLayoutBinding{};
    uboLayoutBinding.binding = 0;
    uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    uboLayoutBinding.descriptorCount = 1;
    ...
    tlasLayoutBinding.binding = 1;
    tlasLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR;
    tlasLayoutBinding.descriptorCount = 1;
    tlasLayoutBinding.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
    tlasLayoutBinding.pImmutableSamplers = nullptr;

    std::array<VkDescriptorSetLayoutBinding, 2> bindings = {uboLayoutBinding, tlasLayoutBinding};
    ...
    VkResult result = vkCreateDescriptorSetLayout(m_context.getDevice(), &layoutInfo, nullptr, &m_descriptorSetLayout);
    CRF_ASSERT_MSG(result == VK_SUCCESS, "Failed to create ray query descriptor set layout");
}
...
void VulkanPipeline::createPipelineLayout(VkDescriptorSetLayout descriptorSetLayout) {
    ...
    pushConstantRange.size = 16 * sizeof(f32); // mat4 model matrix
...
void VulkanPipeline::createSkyPipeline() {
    ...
    VkResult result = vkCreateGraphicsPipelines(m_context.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_skyPipeline);
    CRF_ASSERT_MSG(result == VK_SUCCESS, "Failed to create sky pipeline");
